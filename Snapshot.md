...
**Phase:** `CPM V2 — Module PACKAGED (Create Module + Validate Module done); next = register via cpm-setup`
**Health:** `Active`
...
## Next Action
**Phase 3 packaging is DONE — CPM V2 is a packaged, validated BMad module.** Create Module (CM) generated `skills/cpm-setup/`: `assets/module.yaml` (code `cpm` / "Cinematic Production Module" / 1.0.0 / 5-agent roster lifted from each `customize.toml` / two install-time defaults `target_model`+`default_shard_duration`), `assets/module-help.csv` (42 rows = every capability across all 13 skills + the setup row; 42 globally-unique menu codes; `preceded-by` create→update/validate ordering), the generic setup `SKILL.md`, and the three merge scripts. Validate Module (VM) GREEN: `validate-module.py` → **status PASS / 0 findings**; a 13-agent adversarial quality pass → 9 accurate / 4 minor (all fixed). **NEXT = register/install:** run the **`cpm-setup`** skill — it merges the help rows into `{project-root}/_bmad/module-help.csv`, writes the `cpm` section into `_bmad/config.yaml`, and creates output dirs. Then deploy skills to `.claude/skills/` (auto-discovery by skill name) and run the real end-to-end handshake (Completion Criteria #3). Create-Module open items (c)–(g) ALL RESOLVED this session.
...
### Phase 3: Module Packaging ← IN PROGRESS (CM + VM done; register remains)
...
- [ ] Register CPM V2 — run `cpm-setup` (merges module-help.csv → `_bmad/module-help.csv`, writes `cpm` config section); then deploy skills to `.claude/skills/` for auto-discovery by skill name
...
- [ ] Module registered (run `cpm-setup`) + skills deployed to `.claude/skills/`
