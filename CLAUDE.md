...
> ⚠ **CPM is mid-rebuild (V2) — the `/cpm-*` and `/bmad-agent-cpm-*` commands below are vestigial v1 stubs and are NOT live.** Only `cpm-orchestrator` exists so far (as a v2 skill in `skills/`). See `_bmad-output/cpm-build-plan-2026-06-28.md` for real status (~12% built). These listings will be regenerated by Create Module (CM) when the module is actually packaged.
