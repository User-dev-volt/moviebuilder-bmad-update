...
planDocFolder: '{bmb_creations_output_folder}/workflows/{new_workflow_name}'
workflowPlanFile: '{planDocFolder}/workflow-plan-{new_workflow_name}.md'
...
## ⚠️ CRITICAL — TWO PATHS, DO NOT CONFUSE THEM

| Variable | Purpose | Location |
|----------|---------|----------|
| `planDocFolder` | Where the **plan document** lives (build artifact) | Always `_bmad-output/bmb-creations/workflows/{name}/` |
| `targetWorkflowPath` | Where the **actual workflow files** are built | Set during Classification (step-02) — read from `{workflowPlanFile}` |

**For module-based workflows (CPM, CIS, BMB, etc.), `targetWorkflowPath` is the MODULE'S folder — e.g., `_bmad/cpm/workflows/scene-create/` — NOT `_bmad-output/bmb-creations/`.**

Confusing these two paths caused CRITICAL validation failures in past builds (suite-validation CRITICAL X2: workflow stuck in bmb-creations output).

**Before creating ANY folders or files, complete step 1 below to read the correct `targetWorkflowPath` from the plan.**
...
**FIRST — Read targetWorkflowPath from the plan:**

Load `{workflowPlanFile}` and extract `targetWorkflowPath` from the Classification Decisions section. This is the folder where ALL workflow files will be created. It is NOT the same as `{planDocFolder}`. For module-based workflows it will be something like `_bmad/cpm/workflows/scene-create/`. Store this as your working `targetWorkflowPath` for the rest of this step and steps 08–09.
...
**IMPORTANT:** Always use `targetWorkflowPath` read from the plan (step 1 above), not `{planDocFolder}`.
- For standalone/custom: typically `_bmad/custom/src/workflows/{workflow_name}/`
- For module-based (CPM, CIS, BMB, etc.): `_bmad/{module}/workflows/{workflow_name}/`
